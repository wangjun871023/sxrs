...
public class InfoController {
	...
	public @ResponseBody Map<String, Object> uploadInfoImage(
			MultipartFile file, HttpServletRequest request, String title,
			String width, String height) {
		try {
			BaseForm tmp = new BaseForm();
			infoService.uploadInfoImage(file, request, title, width, height,
					tmp);
			tmp.setSuccess(true);
	 ...
	 * Preparable二次部分绑定的效果,先根据form的id从数据库查出Example对象, 再把Form提交的内容绑定到该对象上。
	 * 因为仅update()方法的form中有id属性，因此本方法在该方法中执行.
	 */
	@ModelAttribute("preEditInfo")
	public InfoEntity preEditInfo(
	...
	public @ResponseBody Map<String, Object> showInfo(String id) {
		try {
			InfoEntity info = infoService.getInfoById(id);
			if(info!=null){
				info.setSuccess(true);
				return ResponseUtils.sendBaseForm(info);
			}
		} catch (Exception e) {
			e.printStackTrace();
			logger.error(e, e);
